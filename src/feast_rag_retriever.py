...
class FeastRagRetriever:
    ...
    def __init__(self, repo_path: Optional[str] = None):
        if not FEAST_AVAILABLE:
            raise ImportError(
                "Feast is required for FeastRagRetriever. Install with: pip install feast"
            )
        ...
        try:
            if self.repo_path.exists():
                ...
                raise FileNotFoundError(
                    f"Feast repository not found at {self.repo_path}"
                )
            ...
            if self.vector_store and self.embedding_model:
                ...
                for i in range(len(chunk_texts)):
                    ...
                    similarity_score = 1.0 - (
                        i * 0.1
                    )  # Simple approximation based on rank
                    ...
                    if similarity_score >= similarity_threshold:
                        doc_data = {
                            ...
                            "chunk_text": (
                                chunk_texts[i] if i < len(chunk_texts) else ""
                            ),
                            "document_title": (
                                document_titles[i]
                            ...
                            "chunk_index": (
                                chunk_indices[i] if i < len(chunk_indices) else 0
                            ),
                            ...
                            "metadata": {
                                ...
                                "file_path": (
                                    file_paths[i] if i < len(file_paths) else ""
                                ),
                            },
                        }
                ...
                logger.info(
                    f"Retrieved {len(documents)} documents using Feast vector store"
                )
                return documents
    ...
    async def retrieve_with_feast_features(
        ...
        candidate_docs = await self.retrieve_documents(
            ...
            for i, doc in enumerate(candidate_docs):
                if i < len(feature_vector):
                    ...
                    doc["similarity_score"] = float(
                        row.get("similarity_score", doc["similarity_score"])
                    )
    ...
    def get_feature_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = {"feature_views": [], "entities": [], "total_features": 0}
