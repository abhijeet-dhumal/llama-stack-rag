...
from config.settings import get_settings, get_feast_repo_path, get_ollama_url
...
class FeastRAGPipeline:
    ...
    async def process_document(self, file_path: str, original_filename: str = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for i, (chunk, embedding) in enumerate(zip(chunks, embeddings)):
                ...
                feature_record = {
                    ...
                    "created_timestamp": pd.Timestamp.now(tz='UTC')  # Required for materialization
                }
    ...
    async def _materialize_features_to_feast(self, feature_data: List[Dict], source_file_path: str) -> None:
        ...
        try:
            ...
            if parquet_path.exists():
                ...
                logger.info(f"Appended {len(feature_data)} features to existing {parquet_path} (total: {len(combined_df)})")
            else:
                # Create new parquet file
                df.to_parquet(parquet_path, index=False)
                logger.info(f"Created new parquet file with {len(feature_data)} features at {parquet_path}")
            
            # Update the FileSource to point to our parquet file
            # Note: In production, you'd typically update feature_definitions.py, 
            # but for this demo we'll use the existing one and ensure the path matches
            
            # Materialize from the parquet file to online store
            # Get current time for materialization window
            ...
            logger.info(f"Successfully materialized {len(feature_data)} features to Feast online store")
        ...
        try:
            ...
            for i, feature_record in enumerate(feature_data):
                transformed_record = {
                    ...
                    "document_id": feature_record["document_id"],  # Keep our document_id
                    "vector": feature_record["embedding"],  # Rename to 'vector' as in official
            ...
            logger.info(f"Using official Feast write_to_online_store method for {len(df)} records")
            ...
            try:
                ...
                logger.info(f"Successfully stored {len(feature_data)} features using official Feast method")
                
            except Exception as store_error:
                error_msg = str(store_error)
                
                # Check if it's still a collection not found error after ensuring it exists
                if "collection not found" in error_msg.lower() or "describecollectionexception" in error_msg.lower():
                    logger.error("Collection still not found after recreation attempt")
                    raise Exception("Failed to create or access Milvus collection. Please restart the server.")
                else:
        ...
        try:
            ...
            if os.path.exists(milvus_path):
                ...
                if not client.has_collection(collection_name):
                    ...
                    import feast
                    self.feast_store = feast.FeatureStore(repo_path="feast_feature_repo")
                    logger.info("Collection should now exist")
                else:
                    logger.debug("Collection exists")
                    client.close()
            else:
                logger.info("Milvus database doesn't exist, will be created on first write")
                
        except Exception as e:
            logger.warning(f"Could not verify collection existence: {e}")
            # Don't raise, let the write operation handle it
    
    async def _retrieve_with_feast_official_method(self, question: str, top_k: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Use official Feast retrieval method
            logger.info(f"Using official Feast retrieve_online_documents_v2 for query: {question[:50]}...")
            ...
            for _, row in context_data.iterrows():
                doc = {
                    ...
                    "similarity_score": float(1.0 - row.get("distance", 0.0))  # Convert distance to similarity
                }
                documents.append(doc)
            
            logger.info(f"Retrieved {len(documents)} documents using official Feast method")
            return documents
    ...
    async def query_documents(
        ...
        try:
            ...
            # Use official Feast retrieval (file-based milvus-lite)
    ...
    def _prepare_context(self, documents: List[Dict]) -> str:
        ...
        for i, doc in enumerate(documents, 1):
            score_text = f" (similarity: {doc['similarity_score']:.3f})" if doc.get('similarity_score') else ""
            context_parts.append(
                f"Document {i}{score_text}:\n{doc['text']}\n"
            )
        return "\n".join(context_parts)
    ...
    async def get_database_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                unique_documents = result_df['document_title'].nunique() if chunk_count > 0 else 0
            ...
            try:
                ...
                if len(context_data) > 0:
                    ...
                    for _, row in context_data.iterrows():
                        ...
                        chunk_count = len(context_data[context_data["document_title"] == title])
                        
                        # Check if we already have this document
                        existing = next((doc for doc in documents if doc["title"] == title), None)
                        if not existing:
                            documents.append({
                                "title": title,
                                "document_type": title.split('.')[-1] if '.' in title else 'md',
                                "chunk_count": chunk_count,
                                "file_path": file_path if not file_path.startswith("/var/folders") else f"uploads/{title}",
...
Answer:"""
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        if self.feast_retriever:
            try:
                status["feast_statistics"] = self.feast_retriever.get_feature_statistics()
                ...
                try:
                    ...
                    try:
                        ...
                        if hasattr(self.feast_store, 'delete_from_online_store'):
                            ...
                            logger.info("Successfully cleared using Feast delete method")
                        else:
                            logger.info("delete_from_online_store not available, using alternative approach")
                            raise AttributeError("delete_from_online_store not available")
                            
                    except (AttributeError, NotImplementedError) as e:
                        logger.info(f"Feast delete method not available: {e}, trying direct Milvus approach")
                        ...
                        milvus_path = "feast_feature_repo/data/online_store.db"
                        if os.path.exists(milvus_path):
                            logger.info("Using direct collection drop/recreate approach")
                            client = MilvusClient(uri=milvus_path)
                            collection_name = "rag_document_embeddings"
                            ...
                            logger.info("Immediately recreating collection after drop...")
                            try:
                                import subprocess
                                ...
                                if result.returncode == 0:
                                    logger.info("Successfully recreated empty collection")
                                    # Reinitialize feast store to pick up new collection
                                    import feast
                                    self.feast_store = feast.FeatureStore(repo_path="feast_feature_repo")
                                    logger.info("Collection ready for immediate uploads")
                                else:
                                    logger.error(f"Failed to recreate collection: {result.stderr}")
                                    logger.info("Collection will be recreated on next upload attempt")
                                    
                            except Exception as recreate_error:
                                logger.error(f"Error recreating collection: {recreate_error}")
                                logger.info("Collection will be recreated on next upload attempt")
                        else:
                            logger.warning(f"Milvus database file not found: {milvus_path}")
