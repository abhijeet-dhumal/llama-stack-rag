...
async def initialize_pipeline():
    ...
    try:
        ...
        feast_pipeline = pipeline
        batch_processor = None
        logger.info(
            f"Feast RAG Pipeline initialized successfully: {feast_pipeline is not None}"
        )
...
class QueryRequest(BaseModel):
    ...
    context_limit: int = Field(
        5, ge=1, le=20, description="Maximum number of context chunks to use"
    )
...
async def health_check():
    ...
    try:
        ...
        if (
            hasattr(feast_pipeline, "feast_retriever")
            and feast_pipeline.feast_retriever
        ):
            ...
            return {"status": "healthy", "message": "Feast RAG pipeline is running"}
    except Exception as e:
        logger.error(f"Health check failed: {str(e)}")
        return {"status": "unhealthy", "message": f"Feast RAG pipeline error: {str(e)}"}
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix=file_extension) as tmp_file:
        try:
            ...
            if result.get("status") == "error":
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to ingest document: {error_msg}"
                )
            ...
            logger.info(
                f"Processed: {file.filename} with {chunks_created} chunks (status: {result.get('status')})"
            )
...
async def query_documents(request: QueryRequest):
    ...
    try:
        ...
        result = await feast_pipeline.query_documents(
            request.question, top_k=request.context_limit
        )
        ...
        adapted_result = {
            ...
            "relevance_scores": [
                doc.get("similarity_score", 0.0) for doc in context_docs
            ],
        }

        logger.info(
            f"Query processed successfully, found {adapted_result['context_used']} relevant chunks using feast"
        )
...
async def list_documents():
    ...
    try:
        ...
        return {
            ...
            "collection_name": documents_data.get(
                "collection_name", "rag_document_embeddings"
            ),
