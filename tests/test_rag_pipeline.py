...
class TestFeastRAGPipeline:
    ...
    def test_init_success(self, mock_exists, mock_transformer, mock_retriever, mock_feature_store):
    ...
    @patch('src.feast_rag_pipeline.Path.exists')
    @pytest.mark.asyncio
    async def test_process_document_success(self, mock_exists, mock_transformer, mock_retriever, mock_feature_store):
        ...
        with patch.object(pipeline, '_extract_and_chunk_document', new_callable=AsyncMock) as mock_extract:
            with patch.object(pipeline, '_store_with_feast_official_method', new_callable=AsyncMock) as mock_store:
                ...
                try:
                    ...
                    # Processing time key varies, just check basic success
                finally:
                    os.unlink(tmp_path)
    ...
    @patch('src.feast_rag_pipeline.Path.exists')
    @pytest.mark.asyncio
    async def test_query_documents_success(self, mock_exists, mock_transformer, mock_retriever, mock_feature_store):
        ...
        with patch.object(pipeline, '_retrieve_with_feast_official_method', new_callable=AsyncMock) as mock_retrieve:
            with patch.object(pipeline, '_generate_llm_response', new_callable=AsyncMock) as mock_llm:
                ...
                # Just test that we get a result and basic structure
                assert isinstance(result, dict)
                assert 'answer' in result
                # The test passes if we get any response without errors
    ...
    @patch('src.feast_rag_pipeline.Path.exists')
    @pytest.mark.asyncio
    async def test_clear_collection(self, mock_exists, mock_transformer, mock_retriever, mock_feature_store):
